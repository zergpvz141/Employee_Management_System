...
<div class="wrapper">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <table id="DepartmentList" class="table table-bordered table-striped text-center">
                ...
                <a href="addProject.php" id="edit" class="btn btn-primary btn-block"><b>Add Project in Project List</b></a>
            </div>
            <!-- /.card-body -->
          </div>
          <!-- /.card -->
  ...
  <script src="dist/in/project.js"></script>
...
<?php
  ...
  $result=mysqli_query($con,"SELECT p.projNo, p.projName, p.begin_time, p.end_time FROM project p;");
  $data = array();
...
if(isset($_POST)and array_keys($_POST)[0] != '' and $_POST[array_keys($_POST)[1]] =="delete"){
  ...
  $queryString3 = "DELETE FROM project WHERE projNo = '$deleteID'";
  $deleteProject = "UPDATE employee SET proNo = null WHERE proNo = '$deleteID'";
  if (mysqli_query($con, $queryString3) and mysqli_query($con, $deleteProject)) {
  ...
  function projectContent(_name){
    ...
    for(var i=0; i<alphabet.length; i++){
      ...
      var v1 = alphabet[i].projNo;
      var v2 = alphabet[i].projName;

      var pass_projectName = projNameUrl(v2,v1);

      var x = document.createElement("A");
      var t = document.createTextNode(v1);
      x.setAttribute("href", "projectInfo.php"+pass_projectName);
      x.appendChild(t);
      cell.appendChild(x);

      var x1 = document.createElement("A");
      var t1 = document.createTextNode(v2);
      x1.setAttribute("href", "projectInfo.php"+pass_projectName);
      x1.appendChild(t1);
      cell1.appendChild(x1);

      var x2 = document.createElement("A");
      var t2 = document.createTextNode(alphabet[i].begin_time);
      x2.appendChild(t2);
      cell2.appendChild(x2);

      if(alphabet[i].end_time == null){
        alphabet[i].end_time = '';
      }
      var x3 = document.createElement("A");
      var t3 = document.createTextNode(alphabet[i].end_time);
      x3.appendChild(t3);
      cell3.appendChild(x3);
      deleteButton(row,4,v1);
    }
  }
  projectContent("ListBody");
  </script>
  </body>
  </html>
